...
const el = {
  ...
  settingsSupabaseUrl: document.getElementById("settingsSupabaseUrl"),
  settingsSupabaseAnonKey: document.getElementById("settingsSupabaseAnonKey"),
  settingsStoreId: document.getElementById("settingsStoreId"),
...
function updateAuthStatus(message) {
  if (!el.authStatusEl) return;
  el.authStatusEl.textContent = message;
}

function getCloudStoreId() {
  return (settings.storeId || DEFAULT_SETTINGS.storeId || "miss-revival-main").toString().trim();
}
...
function applyPersistedSnapshot(payload) {
  if (!payload || typeof payload !== "object") return;
  suppressCloudSave = true;
  sales = Array.isArray(payload.sales) ? payload.sales.map(normalizeSaleShape) : sales;
  vendors = Array.isArray(payload.vendors) ? payload.vendors : vendors;
  vendorPayments = Array.isArray(payload.vendorPayments) ? payload.vendorPayments : vendorPayments;
  expenses = Array.isArray(payload.expenses) ? payload.expenses.map(normalizeExpenseShape) : expenses;
...
async function ensureSupabaseClient() {
  ...
  if (!window.supabase || typeof window.supabase.createClient !== "function") {
    updateAuthStatus("Supabase library failed to load.");
    return null;
  }
  if (supabaseClient) return supabaseClient;
  supabaseClient = window.supabase.createClient(settings.supabaseUrl.trim(), settings.supabaseAnonKey.trim());
...
async function pullCloudSnapshot() {
  ...
  const { data, error } = await client
    ...
    .maybeSingle();
  if (error) {
    updateAuthStatus(`Cloud read failed: ${error.message}`);
    return false;
  }
...
async function pushCloudSnapshot() {
  ...
  const storeId = getCloudStoreId();
  const payload = getPersistedSnapshot();
  ...
  if (error) {
    updateAuthStatus(`Cloud sync failed: ${error.message}`);
    return;
  }
  updateAuthStatus(`Synced to cloud as ${currentUser.email || "user"} (${storeId}).`);
}
...
function queueCloudSave() {
  ...
  cloudSaveTimeout = setTimeout(() => {
    pushCloudSnapshot().catch(() => {});
  }, 300);
}

async function syncCloudNow() {
  if (!currentUser) {
    updateAuthStatus("Sign in first to sync cloud data.");
    return;
  }
  const pulled = await pullCloudSnapshot();
  if (!pulled) await pushCloudSnapshot();
}
...
async function signInWithEmail() {
  ...
  if (error) return window.alert(`Sign in failed: ${error.message}`);
  currentUser = data.user || null;
  updateAuthStatus(`Signed in as ${currentUser?.email || "user"}. Syncing...`);
  await syncCloudNow();
}
...
async function signUpWithEmail() {
  ...
  if (error) return window.alert(`Sign up failed: ${error.message}`);
  window.alert("Account created. Check email if confirmation is enabled, then sign in.");
}
...
async function signOutCloud() {
  ...
  currentUser = null;
  updateAuthStatus("Signed out. App is using local browser storage.");
}
...
function applySettingsToUI() {
  ...
  if (!hasCloudConfig()) {
    updateAuthStatus("Not signed in. Add Supabase URL/Anon Key in Settings to enable cloud login.");
  }
}
...
function syncSettingsForm() {
  ...
  if (el.settingsSupabaseUrl) el.settingsSupabaseUrl.value = settings.supabaseUrl || "";
  if (el.settingsSupabaseAnonKey) el.settingsSupabaseAnonKey.value = settings.supabaseAnonKey || "";
  if (el.settingsStoreId) el.settingsStoreId.value = settings.storeId || DEFAULT_SETTINGS.storeId;
...
function setupEvents() {
  if (el.authSignInBtn) {
    el.authSignInBtn.addEventListener("click", () => {
      signInWithEmail().catch((error) => window.alert(`Sign in failed: ${error.message}`));
    });
  }
  if (el.authSignUpBtn) {
    el.authSignUpBtn.addEventListener("click", () => {
      signUpWithEmail().catch((error) => window.alert(`Sign up failed: ${error.message}`));
    });
  }
  ...
  if (el.cloudSyncNowBtn) {
    el.cloudSyncNowBtn.addEventListener("click", () => {
      syncCloudNow().catch((error) => updateAuthStatus(`Cloud sync failed: ${error.message}`));
    });
  }
  ...
  el.settingsForm.addEventListener("submit", (event) => {
    ...
    settings = {
      ...
      supabaseUrl: (form.get("settingsSupabaseUrl") || "").toString().trim(),
      supabaseAnonKey: (form.get("settingsSupabaseAnonKey") || "").toString().trim(),
      storeId: (form.get("settingsStoreId") || DEFAULT_SETTINGS.storeId).toString().trim() || DEFAULT_SETTINGS.storeId,
...
async function initializeCloudSession() {
  ...
  if (error) {
    updateAuthStatus(`Auth session error: ${error.message}`);
    return;
  }
  ...
  if (currentUser) {
    ...
    updateAuthStatus("Not signed in. App is using local browser storage.");
  }

  client.auth.onAuthStateChange((_event, session) => {
    currentUser = session?.user || null;
    if (currentUser) {
      updateAuthStatus(`Signed in as ${currentUser.email || "user"}.`);
      syncCloudNow().catch(() => {});
    } else {
      updateAuthStatus("Signed out. App is using local browser storage.");
    }
...
initialize().catch((error) => {
  updateAuthStatus(`Initialization error: ${error.message}`);
});
